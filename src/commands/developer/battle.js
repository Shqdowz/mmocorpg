...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    async function PushMonster(name, group, time, from) {
      ...
      if (name == "Shelldon" || name == "Wolf") {
        const oldPet = players.find(
          (p) => p.name == name && p.thresholds.ownerId == from.id
        );

        if (oldPet) {
          players = players.filter((p) => p != oldPet);
          allies = allies.filter((p) => p != oldPet);
          allAllies = allAllies.filter((p) => p != oldPet);
        }
      }

      const newEnemy = await Monster.findOne({ name: name });
      ...
      let enemy = {
        ...
        thresholds: newEnemy.thresholds,
    ...
    // If the command is run outside a root text channel
    ...
    if (authorProfile.party) {
      ...
      if (authorProfile.party.members.length > 1) {
        ...
        for (const member of authorProfile.party.members) {
          if (!member.ready) {
            // If not all party members are readied up
            return await interaction.reply({
          ...
          await PushPlayer(
            ...
            await client.users.fetch(member.user.userId),
            1,
            member.user,
            member.user.speed,
            ToFixedFloat(1 / member.user.speed),
            ToFixedFloat(1 / member.user.speed)
          );
        }
      } else {
        await PushPlayer(
          ...
          authorProfile,
          authorProfile.speed,
          ToFixedFloat(1 / authorProfile.speed),
          ToFixedFloat(1 / authorProfile.speed)
        );
      }
    } else {
    ...
    const monsterAmount = players.length * 2;
    ...
    async function HandleTurn(player) {
      ...
      async function HandleEffect(player, target, effect) {
        ...
        switch (effect.type) {
          ...
          case "Speed":
            ...
            target.interval = ToFixedFloat(1 / target.speed);
        ...
        if (effect.duration == 0) {
          ...
          if (effect.type == "Speed") {
            target.speed = ToFixedFloat(target.speed - effect.value);
            target.interval = ToFixedFloat(1 / target.speed);
          }
        }
      }
      ...
      async function CalculateEffect(range, increase) {
        if (range.length == 2) {
          ...
          return ToFixedFloat(range[0] * increase);
        }
      }
      ...
      async function HandleActive(active, isFromActive) {
        ...
        if (player.thresholds["Active Ace"]) {
          player.profile.copy.gear.active[active] += 1;
        }
        ...
        switch (active) {
          case "Boombox":
            for (const victim of opponents) {
              staticDamage = await CalculateEffect(
                [14, 18],
                (overtime ? 2 : 1) *
                  (0.8 + 0.2 * player.profile.copy.gear.active[active])
              );
              await AddEffect(
                player,
                victim,
              ...
              if (Math.random() <= 0.5) {
                stun = await CalculateEffect(
                  [0.4],
                  0.95 + 0.05 * player.profile.copy.gear.active[active]
                );
          ...
          case "Snow Globe":
            for (const victim of opponents) {
              ...
              speed = await CalculateEffect(
                [-0.15],
                0.9 + 0.1 * player.profile.copy.gear.active[active]
              );
                  ...
                  (overtime ? 2 : 1) *
                    (0.8 +
                      0.2 *
                        ...
                        ).profile.copy.gear.weapon["Wolf Stick"])
                );
                stun = await CalculateEffect(
                  [0.12],
                  0.995 +
          ...
          case "Grow": // toxic sapling
            ...
            damageIncrease = ToFixedFloat(0.16 * player.thresholds.growStacks);
            damageIncrease =
              1 +
              (await CalculateEffect(
                [damageIncrease],
                  ...
                  (overtime ? 2 : 1) *
                    (0.8 +
                      0.2 *
                        allPlayers.find(
                          (p) => p.id == player.thresholds.ownerId
                        ).profile.copy.gear.active["Shelldon"])
                );
                stun = await CalculateEffect(
                  [0.12],
                  0.95 +
          ...
          switch (passive) {
            case "Active Ace":
              chance = await CalculateEffect(
                [0.1],
                0.8 + 0.2 * player.profile.copy.gear.passive[passive]
              );
            ...
            case "Zap in a Box":
              ...
              stun = await CalculateEffect(
                [0.12],
                0.95 + 0.05 * player.profile.copy.gear.passive[passive]
              );
      ...
      async function HandleWeapon(weapon) {
        ...
        if (player.thresholds["Bunch of Dice"]) {
          player.profile.copy.gear.weapon[weapon] += 1;
        }
        ...
        switch (weapon) {
          ...
          case "Medicine Ball":
            ...
            for (const victim of teammates) {
              staticHealing = await CalculateEffect(
                [15, 15],
                (overtime ? 0.5 : 1) *
                  (0.8 + 0.2 * player.profile.copy.gear.weapon[weapon])
              );
              await AddEffect(
                player,
                victim,
          ...
          case "Monster Slugger":
            ...
            for (const victim of opponents) {
              if (
                ...
                stun = await CalculateEffect(
                  [1],
                  0.95 + 0.05 * player.profile.copy.gear.weapon[weapon]
                );
                await AddEffect(player, victim, "Stun", weapon, stun, 1);

                speed = await CalculateEffect(
                  [-0.22],
                  0.9 + 0.1 * player.profile.copy.gear.weapon[weapon]
                );
          ...
          case "Portable Portal":
            ...
            setTimeout(async () => {
              ...
              player.profile.copy.gear.active[equippedOn1] = temp;
            }, 50);
            break;
          case "Spinsickle":
          ...
          case "Alarm Bell":
            if (player.thresholds.hit) {
              ...
              if (enemies.length < 6) {
                ...
                await thread.send({
                  content: `**${player.name}** got hit! It spawned **1 ${monster}**.`,
                });
              } else {
                await thread.send({
      ...
      player.next = ToFixedFloat(player.next + player.interval);
        ...
        if (Math.ceil(Math.random() * chance) == 1) {
          const unlocked = [
            ...["active", "passive", "weapon"].flatMap((type) =>
              FilterGear(ally.profile.copy.gear[type])
            ),
